...
### 支柱 5 · 矩阵 + 配方卡 + 单因子下钻（流程图之外的三层解构）

流程图给方向感，但定量感弱、看不到单因子。需补三个视图：
...
**B+ · 矩阵单元下钻 → 单因子多标签卡组（v0.2 升级）**

矩阵层只看到 archetype 占比（"残差动量 35%"），但不知道这 35% 是由哪些具体因子组成。本层升级让矩阵 cell **可点击下钻**，浮出抽屉显示该 (archetype × 输出) 组合下的所有构成因子。每张因子卡显示 **9 维高密度信息**：

| 卡片维度 | 内容 | 视觉编码 |
|---|---|---|
| 因子名 | mono 字体的命名因子（如 Zhouwei0018_Large） | 标题栏 |
| 状态徽章 | 🟢红利 / 🟡震荡 / 🔴失效 / 🔵在研 | 右上角彩色徽章 |
| 6 metric 网格 | IC20 / IC60 / IR60 / 夏普 / 回撤 / 换手 | 3×2 紧凑布局 |
| 2026 实盘 | 该因子当期净收益 | 底部左，红绿色编码 |
| archetype 内权重 | 该因子在所属 archetype 中占比 % | 底部右，金色 |
| 自由注记 | 构造说明 / 见顶日 / Barra 指纹 | 灰色 italic |

**自动 summary**：抽屉底部统计该 archetype 在该输出下的：状态分布（🟢n 🟡n 🔴n 🔵n）+ **加权 2026 综合贡献 %**（= Σ 因子 rt2026 × 权重）。

**排序规则**：因子卡按"状态优先级 + 权重"排序：🟢 > 🟡 > 🔵 > 🔴，权重高的靠前。让读者先看主菜健康因子，红色失效因子排后面提示问题。
...
var FACTOR_DB = {
  '因子名_LayerSuffix': {
    arch: 1,           // 所属 archetype id (1-9)
    dom: 'L',          // 所属域 (L/M/S/Mi, 可逗号分隔多域)
    ic20: 0.215,       // 20日 IC
    ic60: 0.771,       // 60日 IC
    ir60: 4.49,        // 60日 IR
    sharpe: 1.69,      // 夏普率
    maxdd: -0.039,     // 最大回撤 (decimal, 如 -0.039 = -3.9%)
    turnover: 0.37,    // 换手率
    state: 'g',        // 状态: g 红利 / y 震荡 / r 失效 / b 在研
    state_label: '红利期',
    rt2026: 0.0112,    // 2026 实盘 (decimal)
    weight: 30,        // 在 archetype 内权重 % (整数)
    note: '现役主力,分钟因子残差替代日线 barra shift'
  },
  // ...
};
```
...
**何时启用 B+ 层（升级判断）**：
- 矩阵 cell 数 ≥ 20（5 输出 × 4 archetype 起步），单纯的 % 数字让人想知道"具体是谁"
- 你有每个因子的实际 metrics 数据（IC/IR/夏普 等），不是凭空捏造
